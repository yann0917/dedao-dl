...
type Audio struct {
	...
	AudioID        string  `json:"audio_id"`         // 音频ID（可作为 article/info 的 audio_alias_id 入参）
	...
	// LogID              interface{} `json:"log_id"`                // 日志ID
...
type OdobAudioDetail struct {
	Detail Audio `json:"detail"`
}
...
// AudioDetailByTopicIDStr gets odob audio info by topic_id_str.
func (s *Service) AudioDetailByTopicIDStr(topicIDStr string) (detail *Audio, err error) {
	adetail := OdobAudioDetail{}
	body, err := s.reqOdobAudioDetail(topicIDStr)
	if err != nil {
		return
	}
	defer body.Close()
